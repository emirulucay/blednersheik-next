...
*/
export default function BuildItem({ name, count }: Model) {
  const details = {
    name: name.replaceAll("-", " "),
    link1: `/builds/${name}/${name}-1.png`,
    link2: `/builds/${name}/${name}-2.png`,
    link3: `/builds/${name}/${name}-3.png`,
    link4: `/builds/${name}/${name}-4.png`,
  };
  ...
  const [selectedImg, setSelectedImg] = useState<Selected>({
    selected: `/builds/${name}/${name}-1.png`,
    otherImages: [`/builds/${name}/${name}-2.png`, `/builds/${name}/${name}-3.png`, `/builds/${name}/${name}-4.png`],
  });
  ...
  const handleImage = (image) => {
    ...
    var fullArray = [
      `/builds/${name}/${name}-1.png`,
      `/builds/${name}/${name}-2.png`,
      `/builds/${name}/${name}-3.png`,
      `/builds/${name}/${name}-4.png`,
    ];
    if (image === "image4") {
    ...
    <div className={`sm:mx-0 cursor-pointer group relative aspect-square w-full h-full`}>
      ...
      <div
        ...
        <div className="bg-13/[0.6] rounded-lg w-full lg:w-3/4 transition duration-300 aspect-square shadow z-40 flex items-center justtify-center">
          <div className="block w-full aspect-square p-4">
            <Image
